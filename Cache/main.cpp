...
*/
    for(int i=0; i< SIZE; i++){
        ...
        if(req_type == 0){
           ...
           printf("11\n");
           cache_blk* tmp = cache_obj->search_cache_blk(blk_type, &new_blk);
           //printf("cmp %d\n",strcmp(tmp->username.c_str(), new_blk.username.c_str()));
           if(blk_type == 0 && tmp->food_id == new_blk.food_id)
                printf("found\n");
            if(blk_type == 1 && strcmp(tmp->username.c_str(), new_blk.username.c_str()))
                printf("found\n");
            if(blk_type == 2 && (tmp->ord_id == new_blk.ord_id && !strcmp(tmp->username.c_str(), new_blk.username.c_str())))
                printf("found\n");
        }
        else{
